...
class CVProc {
  ...
  _dispatch(event: {
    ...
    console.log(`_dispatch msg ${msg} ${this._status}`);
    ...
    return new Promise((res, rej) => {
      let interval = setInterval(() => {
        console.log(`_dispatch promise ${this} ${this._status}`);

        const status = this._status[msg];

        if (status[0] === "done") {
          res(status[1] as MessageEvent);
        }

        if (status[0] === "error") {
          rej(status[1] as ErrorEvent);
        }
